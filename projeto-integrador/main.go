...
func main() {
	...
	http.HandleFunc("/cadastro", renderCadastroPaciente) // Nova rota para a página de cadastro
...
func adicionarPaciente(w http.ResponseWriter, r *http.Request) {
	...
	paciente := Paciente{
		...
		EstaSituacaoDeRua: estaSituacaoRua,
	}

	cadastraPaciente(paciente)
	http.Redirect(w, r, "/paciente", http.StatusSeeOther)
}

func renderCadastroPaciente(w http.ResponseWriter, r *http.Request) {
	err := templates.ExecuteTemplate(w, "cadastro.html", nil)
	if err != nil {
		http.Error(w, "Erro ao renderizar template", http.StatusInternalServerError)
		log.Println("Erro ao renderizar template:", err)
	}
...
func fazConexaoComBanco() *sql.DB {
	...
	_, err = database.Exec("CREATE TABLE IF NOT EXISTS paciente (id SERIAL PRIMARY KEY, nome VARCHAR(255) UNIQUE NOT NULL, cpf VARCHAR(15) UNIQUE NOT NULL, data_nascimento VARCHAR(12), telefone_celular VARCHAR(20), sexo VARCHAR(10), esta_fumante BOOLEAN, faz_uso_alcool BOOLEAN, esta_situacao_rua BOOLEAN)")
	if err != nil {
...
func cadastraPaciente(paciente Paciente) {
	...
	if err != nil {
		fmt.Println("Erro ao cadastrar paciente:", err)
	}
}

func buscaPacientePorNome(nome string) Pacientes {
	busca, err := db.Query("SELECT * FROM paciente WHERE nome LIKE '%' || $1 || '%'", nome)
	if err != nil {
	...
	for busca.Next() {
		...
		err = busca.Scan(&paciente.Id, &paciente.Nome, &paciente.Cpf, &paciente.DataNascimento, &paciente.Telefone, &paciente.Sexo, &paciente.EstaFumante, &paciente.FazUsoAlcool, &paciente.EstaSituacaoDeRua)
		if err != nil {
...
func alimentaBancoDeDados() {
	...
	jsonFile, err := os.Open("paciente.json")
	if err != nil {
